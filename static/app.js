...
function cardHtml(m){
  ...
  const prem = m.is_premium ? `<span class="card-prem">💎</span>` : '';
  ...
  const langBadge = langs.length>1
    ? `<span class="card-langs" title="Tillari: ${esc(langs.join(', '))}">🌐 ${langs.length} til</span>` : '';
  const badges = (prem || rate || langBadge) ? `<div class="card-badges">${prem}${rate}${langBadge}</div>` : '';
  return `<a href="/kino/${m.id}" class="card ${pal(m.id)}" onclick="event.preventDefault();openMovie(${m.id})">
    <div class="card-img">${bg}${badges}<span class="card-name">${esc(m.title)}</span>${meta?`<span class="card-meta">${esc(meta)}</span>`:''}</div>
